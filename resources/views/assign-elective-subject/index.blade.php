            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="create-form" class="pt-3" action="{{ route('assign.elective.subject.store') }}"
                            method="POST" novalidate="novalidate" data-success-function="formSuccessFunction">
                            @csrf
                            <div class="row" id="toolbar">
                                <div class="form-group col-sm-12 col-md-3">
                                    <label for="session_year_id" class="filter-menu">{{ __('Session Year') }}</label>
                                ...
                                <div class="form-group col-sm-12 col-md-3">
                                    <label for="class_id" class="filter-menu">{{ __('class') }}</label>
                                    <select name="class_id" id="filter-class-id" class="form-control select2">
                                        <option value="">{{ __('select') . ' ' . __('class') }}</option>
                                        @foreach ($classes as $class)
                                            <option value={{ $class->id }}>
                                                {{ $class->name }} {{ $class->medium->name }} {{ $class->stream->name ?? '' }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-sm-12 col-md-3">
                                    <label for="class_section_id" class="filter-menu">{{ __('section') }}</label>
                                    <select name="class_section_id" id="filter-class-section-id"
                                        class="form-control select2">
                                        <option value="">{{ __('select') . ' ' . __('section') }}</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-sm-12 col-md-3">
                                    ...
                                    <select name="class_subject_id" id="subject_id" class="form-control select2">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <table aria-describedby="mydesc" class='table' id='table_list' data-toggle="table"
                                        ...
                                        data-export-types='["txt","excel"]' data-export-options='{ "fileName": "elective-subject-list-<?= date('d-m-y') ?>"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col" data-field="elective_subjects"
                                                    ...
                                                    {{ __('Selected Subjects') }}
                                                </th>
                                                {{-- <th scope="col" data-field="total_subjects">{{ __('Total Elective
                                                    Subjects') }}</th>
                                                <th scope="col" data-field="total_selected">{{ __('Total Selected Subjects')
                                                    }}</th> --}}
...
@section('script')
    <script>
        $(document).ready(function () {
            ...
            // Handle class change to populate sections and elective subjects
            $('#filter-class-id').on('change', function () {
                var classId = $(this).val();

                // Reset dependent dropdowns
                $('#filter-class-section-id').html('<option value="">{{ __("select") }} {{ __("section") }}</option>');
                $('#subject_id').html('<option value="">{{ __("select") }} {{ __("subject") }}</option>');

                if (classId) {
                    $.ajax({
                        url: "{{ route('assign.elective.subject.get-sections', ':class_id') }}".replace(':class_id', classId),
                        type: "GET",
                        success: function (response) {
                            response.forEach(function (item) {
                                $('#filter-class-section-id').append('<option value="' + item.id + '">' + item.section.name + '</option>');
                            });
                        }
                    });

                    $.ajax({
                        url: "{{ route('assign.elective.subject.get-electives', ':class_id') }}".replace(':class_id', classId),
                        type: "GET",
                        success: function (response) {
                            if (response.length > 0) {
                                response.forEach(function (item) {
                                    $('#subject_id').append('<option value="' + item.id + '">' + item.subject.name + ' (' + item.subject.type + ')</option>');
                                });
                            } else {
                                $('#subject_id').html('<option value="">{{ __("no_data_found") }}</option>');
                            }
                        }
                    });
                }
            ...
            $('#filter-class-section-id, #subject_id, #filter-session-year-id').on('change', function () {
        ...
        function formSuccessFunction(response) {
            setTimeout(() => {
                ...
                // Reset dropdowns
                $('#filter-class-id').val('').trigger('change');
                $('#filter-class-section-id').html('<option value="">{{ __("select") }} {{ __("section") }}</option>');
                $('#elective-subject-id').html('<option value="">{{ __("select") }} {{ __("subject") }}</option>');
            }, 500);
        }
    </script>
@endsection