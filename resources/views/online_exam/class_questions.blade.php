    ...
    <div class="content-wrapper">
        ...
        <div class="row grid-margin">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <form class="pt-3 mt-6 common-validation" id="create-online-exam-questions-form" method="POST"
                                        ...
                                        <select required name="subject_id" id="subject-id" class="form-control">
                                            ...
                                            @foreach ($subjectTeachers as $item)
                                                <option value="{{ $item->subject_id }}"
                                                    ...
                                                    data-user="{{ Auth::id() }}">{{ $item->subject_with_name}}</option>
                                            @endforeach
                                        </select>
                                    @endif
                            ...
                            <div class="bg-light p-4">
                                <div class="form-group">
                                    ...
                                    <textarea class="editor_question" name="question" id="question" required
                                ...
                                <div class="options-data">
                                    <div data-repeater-list="option_data" class="row">
                                        <div class="form-group col-lg-6 col-md-12" data-repeater-item>
                                            <label for="option">{{ __('option') }} <span class="option-number">0</span>
                                                <span class="text-danger">*</span></label>
                                            <textarea class="editor_options" name="option" id="option" required
                                                placeholder="{{__('enter') . ' ' . __('option')}}"></textarea>
                                            <input type="hidden" name="number" value="" class="option-number">
                                            <button type="button" class="btn btn-inverse-danger mt-2 btn-icon remove-option"
                                ...
                                <div class="row mt-4">
                                    <div class="form-group col-md-6">
                                        <div class="form-group">
                                            <label for="answer_select">{{ __('answer') }} <span
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <div class="input-group col-xs-12">
                                            <input type="text" class="form-control file-upload-info" id="image" disabled=""
                                        ...
                                        <div class="d-flex">
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input class="easy" checked="checked" name="difficulty" type="radio"
                                                        value="easy">{{ __('easy') }}
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div id="toolbar" class="row">
                            ...
                            <div class="form-group col-sm-12 col-md-3">
                                <label for="filter-subject-id" class="filter-menu">{{ __('subject') }} <span
                                        class="text-danger">*</span></label>
                                @if (Auth::user()->hasRole('Teacher'))
                                    <select required name="subject_id" id="filter-subject-id"
                                        class="form-control filter-subject-id">
                                        <option value="">-- {{ __('Select Subject') }} --</option>
                                        <option value="data-not-found">-- {{ __('no_data_found') }} --</option>
                                        @foreach ($subjectTeachers as $item)
                                ...
                                @else
                                    <select required name="subject_id" id="school-filter-subject-id"
                                        class="form-control filter-subject-id">
                                        <option value="">-- {{ __('Select Subject') }} --</option>
                                        <option value="data-not-found">-- {{ __('no_data_found') }} --</option>
                                        @foreach ($classSubjects as $item)
                                            <option value="{{ $item->subject_id }}" data-class-id="{{ $item->class_id }}">
                                                {{ $item->subject_with_name}}</option>
                                        @endforeach
                                    </select>
                                @endif
                        ...
                        <table aria-describedby="mydesc" class='table' id='table_list' data-toggle="table"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" data-field="class_name" data-formatter="ClassSectionFormatter">
                                        {{ __('Class') }}</th>
                                    <th scope="col" data-field="subject_name">{{ __('subject') }}</th>
                                    <th scope="col" data-field="question" data-escape="false">{{ __('question')}}</th>
                                    <th scope="col" data-field="options" data-formatter="optionsFormatter">
                                        {{ __('option') }}</th>
                                    <th scope="col" data-field="answers" data-formatter="answersFormatter">
                                        {{ __('answer') }}</th>
