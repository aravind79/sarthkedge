            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('subscriptions-bill-payment.update', $subscriptionBill->id) }}" method="POST"
                            class="edit-form" novalidate="novalidate" data-success-function="formSuccessFunction">
                            @csrf
                            @method('POST')
                            ...
                            <div class="border border-secondary rounded-lg my-4 mx-1">
                                ...
                                <div class="row my-4 mx-1">
                                    ...
                                    <div class="form-group col-md-6 col-sm-12 text-right">
                                        <span
                                            class="billing_cycle btn-gradient-dark p-2">{{ date(data_get($systemSettings ?? null, 'date_format') ?? 'd-m-Y', strtotime($subscriptionBill->subscription->start_date)) }}
                                            -
                                            {{ date(data_get($systemSettings ?? null, 'date_format') ?? 'd-m-Y', strtotime($subscriptionBill->subscription->end_date)) }}</span>
                                    </div>
                                </div>
                                ...
                                <input type="hidden" name="amount"
                                    value="{{ number_format(ceil(($subscriptionBill->amount) * 100) / 100, 2) }}">
                                        ...
                                        @if ($subscriptionBill->transaction && $subscriptionBill->transaction->payment_gateway == 'Cash')
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" name="subscription_transaction[payment_gateway]"
                                                        value="Cash" class="form-check-input payment_type cash" checked>
                                                    {{ __('cash') }}
                                                </label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" name="subscription_transaction[payment_gateway]"
                                                        value="Cheque" class="form-check-input payment_type cheque">
                                                    {{ __('cheque') }}
                                                </label>
                                            </div>
                                        @else
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" name="subscription_transaction[payment_gateway]"
                                                        value="Cash" class="form-check-input payment_type cash">
                                                    {{ __('cash') }}
                                                </label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <label class="form-check-label">
                                                    <input type="radio" name="subscription_transaction[payment_gateway]"
                                                        value="Cheque" class="form-check-input payment_type cheque" checked>
                                                    {{ __('cheque') }}
                                                </label>
                                            </div>
                                        @endif
                                ...
                                <div class="form-group col-sm-12 col-md-4 cheque_input">
                                    ...
                                    <input type="text" name="cheque_number" class="form-control"
                                        placeholder="{{ __('enter_cheque_number') }}"
                                        value="{{ $subscriptionBill->transaction ? $subscriptionBill->transaction->order_id : '' }}"
                                        required>
                                </div>
