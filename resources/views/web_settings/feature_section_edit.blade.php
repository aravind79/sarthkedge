                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-12 text-right">
                                <a href="{{ route('web-settings.feature.sections') }}"
                                    class="btn btn-theme btn-sm">{{ __('back') }}</a>
                            </div>
                        </div>
                        <hr>
                        <form action="{{ route('web-settings-section.update', $featureSection->id) }}" method="POST"
                            class="edit-form pt-3" novalidate="novalidate" enctype="multipart/form-data"
                            ...
                            <div class="row">
                                <div class="form-group col-sm-12 col-md-4">
                                    ...
                                    <input type="text" name="title" value="{{ $featureSection->title }}"
                                        class="form-control" placeholder="{{ __('title') }}" required>
                                </div>
                                <div class="form-group col-sm-12 col-md-8">
                                    <label>{{ __('heading') }} <span class="text-danger">*</span></label>
                                    <input type="text" name="heading" value="{{ $featureSection->heading }}"
                                        class="form-control" placeholder="{{ __('heading') }}" required>
                                </div>
                            </div>
                            <hr>
                                ...
                                <div data-repeater-list="section_data">
                                    @foreach ($featureSection->feature_section_list as $section)
                                        <div class="row file_type_div" id="file_type_div" data-repeater-item>
                                            <input type="hidden" name="id" id="edit-id" value="{{ $section->id }}">
                                            ...
                                            <div class="form-group col-sm-12 col-md-3 file-input" id="file_div">
                                                ...
                                                <input type="file" name="section[0][image]" class="file form-control"
                                                    accept="image/png, image/jpeg, image/jpg, image/webp" placeholder=""
                                                    value="image">
                                                @if (!empty($section->image))
                                                    <input type="hidden" name="section[0][old_image]" class="file form-control"
                                                        value="{{ $section->image }}">
