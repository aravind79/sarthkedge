            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('roles.update', $role->id) }}" class="edit-form-without-reset">
                            @csrf
                            @method('PATCH')
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name" class="form-control" placeholder="{{ __('name') }}"
                                            value="{{ $role->name }}" {{ $role->name == "Teacher" ? 'readonly' : '' }}>
                                    </div>
                                </div>
                                <div class="form-group col-lg-3 col-sm-12 col-xs-12 col-md-3">
                                    <div class="form-check">
                                        <label class="form-check-label">
                                            <input type="checkbox" name="selectall" value="1" class="name form-check-input"
                                                    ...
                                                    <div class="form-check">
                                                        <label class="form-check-label">
                                                            <input type="checkbox" name="permission[]" value="{{ $value->id }}"
                                                                class="form-check-input selectedId child-checkbox"
                                                                data-group="{{ $group }}"
                                                                onchange="updateParentCheckboxState('{{ $group }}')" {{ in_array($value->id, $rolePermissions) ? 'checked' : '' }}>
                                                            {{ $value->name }}
                                                        </label>
                                                    </div>
                                                </div>
