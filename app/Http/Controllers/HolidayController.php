...
class HolidayController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $holidayStartDate = Carbon::parse($request->date);
            $holidayEndDate = $request->end_date ? Carbon::parse($request->end_date) : $holidayStartDate;
            ...
            if ($holidayStartDate->lt($start) || $holidayStartDate->gt($end)) {
                ResponseService::errorResponse('The selected start date must fall within the current session year.');
            }
            if ($holidayEndDate->lt($start) || $holidayEndDate->gt($end)) {
                ResponseService::errorResponse('The selected end date must fall within the current session year.');
            }
            if ($holidayEndDate->lt($holidayStartDate)) {
                ResponseService::errorResponse('End date cannot be before start date.');
            }
