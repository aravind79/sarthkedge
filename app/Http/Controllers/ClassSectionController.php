...
class ClassSectionController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = [
            'total_sections' => $this->classSection->builder()->count(),
            'total_capacity' => $this->classSection->builder()->sum('capacity'),
            'total_strength' => \App\Models\User::role('Student')->where(['status' => 1, 'school_id' => Auth::user()->school_id])->count(),
        ];

        return response(view('class-section.index', compact('classes', 'teachers', 'stats')));
    }

    public function store(Request $request)
    {
        ResponseService::noPermissionThenRedirect(['class-section-create']);
        $request->validate([
        ...
        try {
            ...
            $section = $this->section->builder()->where('name', $request->section_name)->first();
            if (!$section) {
                $section = $this->section->create(['name' => $request->section_name]);
            }

            // 2. Fetch Class to get medium_id
            $class = $this->class->findById($request->class_id);

            // 3. Check if ClassSection already exists
            $classSection = $this->classSection->builder()->where(['class_id' => $request->class_id, 'section_id' => $section->id])->first();
            if ($classSection) {
                ResponseService::errorResponse("This section already exists for the selected class.");
            }

            // 4. Create ClassSection
            ...
            // 6. Give permissions to teacher
            $this->user->findById($request->teacher_id)->givePermissionTo(['class-teacher', 'exam-upload-marks', 'exam-result', 'attendance-list']);

            DB::commit();
            ResponseService::successResponse('Section created successfully');
        } catch (Throwable $e) {
            DB::rollBack();
    ...
    public function show(Request $request)
    {
        ...
        foreach ($res as $row) {
            ...
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            $tempRow['strength'] = $row->students->count();
            $tempRow['capacity'] = $row->capacity ?? 0;
            $tempRow['available_seats'] = max(0, ($row->capacity ?? 0) - $tempRow['strength']);
    ...
    public function edit($id)
    {
        ...
        $teachers = $this->user->builder()->role('Teacher')->get();
        $classSubjects = $this->classSubjects->builder()->where('class_id', $classSection->class_id)->with([
            'subjectTeachers' => function ($q) use ($id) {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $this->classSection->update($id, $request->only('capacity'));
