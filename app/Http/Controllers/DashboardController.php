...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->hasRole('School Admin') || Auth::user()->school_id) {
            ...
            try {
                $upcoming_events = DB::connection('school')->table('holidays')
                    ->select('title', 'date', DB::raw("'holiday' as event_type"), DB::raw("'All Day' as time"))
                    ->whereDate('date', '>=', $today)
                    ->union(
                        DB::connection('school')->table('academic_calendars')
                            ->select('title', 'date', DB::raw("'event' as event_type"), DB::raw("DATE_FORMAT(start_time, '%h:%i %p') as time"))
                            ->whereDate('date', '>=', $today)
                    )
                    ->orderBy('date', 'ASC')
                    ->limit(10)
