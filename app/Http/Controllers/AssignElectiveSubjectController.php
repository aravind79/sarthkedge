...
class AssignElectiveSubjectController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            $classes = $this->class->all(['*'], ['medium', 'sections', 'stream']);
            $session_years = $this->sessionYear->all();

            return view('assign-elective-subject.index', compact('classes', 'session_years'));
        } catch (\Exception $e) {
    ...
    {
        ResponseService::noPermissionThenSendJson('assign-elective-subject-list');
        try {
            $sections = $this->classSection->builder()->where('class_id', $class_id)->with('section')->get();
            return response()->json($sections);
        } catch (\Exception $e) {
            return ResponseService::logErrorResponse($e, 'AssignElectiveSubjectController -> getSectionsByClass method');
        }
    }

    public function getElectivesByClass($class_id)
    {
        ResponseService::noPermissionThenSendJson('assign-elective-subject-list');
        try {
            $electives = $this->classSubject->builder()
            ...
            return ResponseService::logErrorResponse($e, 'AssignElectiveSubjectController -> getElectivesByClass method');
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                // Get the elective subject details first
                ...
                if ($electiveSubject->elective_subject_group_id) {
                ...
                foreach ($student_ids as $student_id) {
                    ...
                    // Count how many subjects this student already has in this group (if applicable)
                ...
                if ($successCount > 0) {
                    ...
                    return ResponseService::errorResponse('Elective Subjects are already assigned or limit reached.');
                }
    ...
    public function destroy($id)
    {
        ...
        try {
            $studentSubject = $this->studentSubject->builder()->where('id', $id)->first();
            if (!$studentSubject) {
    ...
    {
        return ResponseService::errorResponse('Not applicable');
    }
